...
contract ItemFactory is ERC1155SupplyCC, AccessControl {
    ...
    uint16 public _maxRarityRoll = 100; //since legendaryRoll is 100, maxRarityRoll is also 100 initally
    ...
    //indexed keyword in claimer, for quick filtering
    ...
    function claim(address claimer, uint256 entropy) external {
        ...
        require(block.timestamp >= _lastClaim[claimer] + 1 days, "WAIL_UNTIL_NEXT_PERIOD");
        ...
        // handle MILK or ITEMS
            ...
            // MILK or ITEMS.
            rewardType = randomNum(entropy) % (uint256(EType.MILK) + 1); //length of the enum (last element + 1)
    ...
    //As both ERC1155 and AccessControl include supportsInterface we need to override them or it will throw some error
