...
contract Milk is ERC20, AccessControl {
    ...
    constructor(
        ...
        _setupRole(CONTRACT_ROLE, systemCheckerContractAddress); //I assume that systemCheckerContractAddress is intended address that will get granted for CONTRACT_ROLE
    }
    ...
    /// @notice grant a user a role
    /// @dev the roles can be granted within constuctor if the role takers already decided
    /// @dev used index based function here in order to reduce function congestion
    /// @dev seperate functions can be used for achive this
    /// @param roleIndex true or false, true represents CONTRACT_ROLE, false represents MASTER_ROLE
    /// @param targetAddress address for granting role to
    function grantRoleByIndex(bool roleIndex, address targetAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(targetAddress != address(0), "INVALID_ADDRESS");

        if(roleIndex) {
            _setupRole(CONTRACT_ROLE, targetAddress);
        } else if(!roleIndex) {
            _setupRole(MASTER_ROLE, targetAddress);
        }
    }
}
